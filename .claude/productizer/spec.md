...
| R39 | `selftest-coverage` check over `check-selftest-coverage.sh` — reads the tools the declared checks and `.github/workflows/checks.yml` actually invoke, and reports which carry a self-test that ANSWERS, by running it rather than by grepping for the flag. Measured 2026-09-06: **4 of 32** repository check tools carry one. It names the other 28 individually and reports ITSELF as a finding rather than exempting the tool doing the counting. The check is RED and therefore voids its own claim - `run-checks.sh` lists `fail` in `VOID_RUN` - so this requirement reads Missing until those 28 exist. That is the work queue, not a gap in the check. NOT asserted: the clause `reaches each exit code it can return` - no self-test here reports which exit codes it drove, and only `check-selftest-coverage.sh` itself demonstrates the clause, across 10 cases over exits 0, 1 and 2 |
| R40 | `selftest-coverage` check over `check-selftest-coverage.sh` — distinct from R39: R39 obliges a self-test to EXIST, this obliges something to RUN it. Reachability is read statically from each check's `command` argv and the workflow's `run:` lines. **CORRECTION, measured 2026-09-06:** an earlier version of this row claimed `check-nothing-merged.sh --selftest` exists. It does not - that script answers `unknown option --selftest` and names the flag only in a header comment. Exactly three scripts dispatched on a self-test flag before this change - `validate-spec.py`, `contradiction-check.py` and `check-acceptance-rows.sh` - the last of which no check and no workflow step reached, and a step was added for it. Like R39 this reads Missing while the check is red. NOT asserted: a self-test reached by a Makefile, a git hook or a person is invisible here, and a declared check whose tool is absent at run time reads as reached while running nothing |
