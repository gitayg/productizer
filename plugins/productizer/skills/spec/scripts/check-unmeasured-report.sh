...
SELFTEST_HYGIENE
  ...
  # entry per case as self_report reads its code off the file the background
  # drive wrote. A literal list would satisfy the reader and prove nothing.
  CODES=""
...
"
  ...
  if [ -n "$MISSING" ]; then
    printf 'check-unmeasured-report: documented exit code(s) no self-test case reached:%s\n' "$MISSING" >&2
    exit 1
  fi
